...
def test_eig_spec_normal_adds_qvecs():
    from LAFigureSpecs import eig_spec

    A = [[1, 0], [0, 1]]
    spec = eig_spec(A, normal=True)
    assert "qvecs" in spec
    # Identity has a 2D eigenspace.
    assert len(spec["qvecs"][0]) == 2


def test_svd_spec_has_sz_and_sigma():
...
def test_svd_spec_rectangular_rank_deficient_adds_left_nullspace():
    from LAFigureSpecs import svd_spec
