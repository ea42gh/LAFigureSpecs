...
def test_legacy_bg_uses_block_padding_for_right_align():
    ...
    assert "fit = (3-2-medium)" in codebefore[0]


def test_legacy_vertical_column_highlights_stay_inside_column_bounds():
    matrices = [
        [None, [[1, 2], [3, 4], [5, 6]]],
    ]
    ...
    codebefore = _legacy_bg_list_to_codebefore(matrices, bg_list)
    assert codebefore
    assert "fit = (1-5-medium) (3-5-medium)" in codebefore[0]
    assert "(1-|5) (4-|6)" not in codebefore[0]
