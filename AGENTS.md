...
This repository is coupled to the sibling `matrixlayout` project:

- `matrixlayout`: layout-only TeX/SVG rendering primitives.
- `la_figures`: linear-algebra algorithms and spec builders that call `matrixlayout`.
...
`la_figures` call sites, specs, tests, docs, and notebooks in the same slice.
...
2. Remove or isolate stale compatibility code only when no `la_figures` caller uses it.
...
1. Rename GE RHS partition fields from `Nrhs` to `n_rhs`.
   Update `matrixlayout` specs/renderers and every `la_figures` spec/call site.
...
7. Refresh docs and notebooks after code migration.
   The `la_figures` documentation should present algorithm-facing APIs; the
...
- Run `la_figures`: full pytest and render smoke where available.
- Include at least one end-to-end `la_figures -> matrixlayout` check for any
